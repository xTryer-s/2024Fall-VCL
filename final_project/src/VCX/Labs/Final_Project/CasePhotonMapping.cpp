...
namespace VCX::Labs::Rendering {
    ...
    CasePhotonMapping::CasePhotonMapping(std::initializer_list<Assets::ExampleScene> && scenes):
        _scenes(scenes),
        _program(
    ...
    CasePhotonMapping::~CasePhotonMapping() {
        _stopFlag = true;
        if (_task.joinable()) _task.join();
    }

    void CasePhotonMapping::OnSetupPropsUI() {
        ...
        if (ImGui::CollapsingHeader("Appearance", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            _resetDirty |= ImGui::SliderInt("Photon Num", &_photon_nums, 400, 200000);
            _resetDirty |= ImGui::SliderInt("Near Photon Num", &_near_photon_nums, 1, 200);
            _resetDirty |= ImGui::SliderInt("Max Bounce", &_maximumPhotonBounce, 1, 15);
            _resetDirty |= ImGui::SliderInt("Photon Size", &_photon_radius, 1, 200);
    ...
    Common::CaseRenderResult CasePhotonMapping::OnRender(std::pair<std::uint32_t, std::uint32_t> const desiredSize) {
        if (_resetDirty) {
            _stopFlag = true;
            if (_task.joinable()) _task.join();
            _pixelIndex = 0;
        ...
        if (! _stopFlag && ! _task.joinable()) {
            ...
            _task = std::thread([&]() {
                ...
                if (_pixelIndex == 0)
                {    
                    _photonMapping.Init_TracePhotons(_intersector, _photon_nums, _maximumPhotonBounce);
                ...
                while (_pixelIndex < std::size_t(width) * height) {
                    ...
                    for (int dy = 0; dy < _superSampleRate; ++dy)
                        for (int dx = 0; dx < _superSampleRate; ++dx) {
                            ...
                            glm::vec3 res = RayTraceWithPhotonMapping(_intersector, initialRay, _maximumDepth, _photonMapping, _near_photon_nums, _photon_radius,_enablekd);
    ...
    void CasePhotonMapping::OnProcessInput(ImVec2 const & pos) {
